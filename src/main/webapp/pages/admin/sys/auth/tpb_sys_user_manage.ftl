...
<div class="col-md-12">
	<div class="portlet box blue">
		...
		<div class="portlet-body">
			<#include "/pages/layout/toolbar.ftl"/> 
...
$(function(){
	datatables = $('#table').dataTable( {  
	    ...
	    "sServerMethod": "POST",
	  	"sAjaxSource": '/admin/sys_authority_user/data?rand='+Math.random(),  
	 	"oLanguage": {  
            "sProcessing": "正在加载数据...",  
            'sSearch': '数据筛选:',  
        ...
        "iDisplayLength": 10, 
	  	...
	  	"aoColumns":[  
	  		...
	  		{ "mDataProp": "username","sTitle" : "用户名称","sClass" : "text-center"  },
	  		{ "mDataProp": "nickname","sTitle" : "用户昵称","sClass" : "text-center"  },
	  		{ "mDataProp": "role_names","sTitle" : "角色","sClass" : "text-center"  },
	  		{ "mDataProp": "department_name","sTitle" : "所属部门","sClass" : "text-center"  },
		...
		"columnDefs": [
	        {
	          ...
	          "render": function(data, type, full) {
		            ...
		            <#list btnList as btn>
		            	html += '		<li>';
		            	if("${(btn.btn_code)!''}" == 'add'){
		            		html += '			<a href="/admin/sys_authority_user/toAdd"><i class="fa fa-plus"></i> ${(btn.btn_name)!''}</a>';
		            	}else if("${(btn.btn_code)!''}" == 'edit'){
		            		html += '			<a href="/admin/sys_authority_user/toAdd?id=' + full.id + '"><i class="fa fa-edit"></i> ${(btn.btn_name)!''}</a>';
		            	}else if("${(btn.btn_code)!''}" == 'forbidden'){
...
function forbidden(id){
	manage("/admin/sys_authority_user/forbidden",{id:id});
}

function normal(id){
	manage("/admin/sys_authority_user/normal",{id:id});
}
